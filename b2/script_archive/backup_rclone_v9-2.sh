...
CHUNK_BUDGET_MB=49999                     # stop script from creating new chunks after ~10 GB (bluehost seems to fail anyhow after 1.5 Gb)
# Target size for each chunk zip (creation batches)
CHUNK_TARGET_MB=1024                      # create zips about this big each
# =================================
...
# ---------- Remote skip check (if already uploaded) ----------
REMOTE_PREFIX="${REMOTE}/${BASENAME}"
if "$RCLONE" lsf "$REMOTE_PREFIX" --include "$UPLOAD_MARKER" --log-level=INFO --log-file="$LOG_FILE" | grep -qx "$UPLOAD_MARKER"; then
  echo "Remote marker found (${REMOTE_PREFIX}/${UPLOAD_MARKER}). This period already uploaded. Skipping." | tee -a "$LOG_FILE"
...
if [[ "$MISSING_COUNT" -eq 0 ]]; then
  ...
  # verify marker exists remotely
  if ! "$RCLONE" lsf "$REMOTE_PREFIX" --include "$UPLOAD_MARKER" --log-level=INFO --log-file="$LOG_FILE" | grep -qx "$UPLOAD_MARKER"; then
    echo "Remote verification failed: marker not found after upload." | tee -a "$LOG_FILE"
...
find "$BACKUP_DIR" -maxdepth 1 -type d -name "*.${PERIOD_TAG}.parts" -mtime +$KEEP_LOCAL_DAYS -print -exec rm -rf {} \; >> "$LOG_FILE" 2>&1 || true
